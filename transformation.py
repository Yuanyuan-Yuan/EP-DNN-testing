...
# stylize_model = StylizeModel(image_size)
# stylize1 = Stylize(style: torch.Tensor, id, stylize_model.transform)

class Stylize(Transformation):
    def __init__(self, style_path, id, trans):
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.category = 'style'
        self.name = 'style_%d' % id
        self.style_path = style_path
        self.trans = trans

    def mutate(self, seed):
        x = seed['x']
        x_ = self.trans(x.to(self.device), self.style_path, self.extent())
        return x_.cpu(), seed['z']

    def extent(self):
        ext = np.random.randint(4, 9) / 10
        return ext

class Perceptual(Transformation):
    def __init__(self, direction, id, trans, max_extent):
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.category = 'perceptual'
        self.name = 'perceptual_%d' % id
    ...
    def mutate(self, seed):
        z = seed['z']
        z_ = z + self.direction * self.extent()
        x_ = self.trans(z_.to(self.device))
        return x_.cpu(), z_.cpu()

    def extent(self):
        ext = np.random.uniform(0, self.max_extent)
        return ext

class ClassPerceptual(Perceptual):
    def __init__(self, direction, id, trans, max_extent):
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.category = 'perceptual'
        self.name = 'perceptual_%d' % id
    ...
    def mutate(self, seed):
        z = seed['z']
        z_ = z + self.direction * self.extent()
        x_ = self.trans(z_.to(self.device), seed['y'].to(self.device))
        return x_.cpu(), z_.cpu()
